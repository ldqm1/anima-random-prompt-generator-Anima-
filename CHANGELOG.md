...
- **QSS 全面重写**：圆角卡片（生成/结果区）、Tab 胶囊选中、输入框聚焦高亮、
  渐变进度条、代码框深色底、滚动条圆角、自定义下拉箭头、列表 hover、手型光标、窗口图标
- **Toast 气泡提示**（`gui_qt/toast.py`）：复制/保存/生成/切换预设/主题切换均弹出
  右下角气泡（成功绿/错误红/信息蓝），QTimer 可靠消失，新气泡替换旧气泡不堆叠
- **按钮闪烁反馈**：复制→"✓ 已复制"、保存→"✓ 已保存"、生成→"✓ 已生成 N 条"，1.2s 恢复

### 配置文档（可搜索 HTML）

- `tools/gen_config_doc.py` 自动提取 508 项配置生成 `docs/配置文档.html`
- 顶部搜索框实时过滤配置表/章节；含快速开始 3 步、7 张界面截图、11 章节导航、
  6 个常见调整效果示例
- GUI 底部栏新增「📖 配置文档」按钮，浏览器打开（打包版内置 docs 资源）

### 文档截图方框修复

- 根因：截图用 offscreen 渲染，offscreen 找不到中文字体 → 截图中文成方框
- 修复：改用真实窗口渲染全部截图，验证实心字形正常（内部笔画 108-589）
...
- 「随机 / 固定」由两个独立复选框改为 **QRadioButton + QButtonGroup 互斥单选**
  （选固定自动取消随机，反之亦然；选固定时启用 seed 输入框，随机时禁用）
- 固定模式收集 seed 值，随机模式 seed 为 None（与引擎语义一致）

### 去除全部折叠（深层嵌套也平铺）

- **根因**：章节内小节虽平铺，但 `forms._build_value` 对深层嵌套 dict
  （如 subcategory_quotas 下的类别、r18_topic_control.topics 下的主题）仍生成
  CollapsibleSection 折叠
- **修复**：嵌套 dict 一律平铺 —— 灰色分组标题（中文名）+ 内容直接构建，
  全页面无任何 ▸/▾ 折叠控件

### 额外要求池显示修复

- ListEditor 对象行字段**未加入布局**（只创建未 addWidget）导致显示异常
- **修复**：`_make_object_row` 把每个字段 widget 加入对象行布局；字段名全部汉化
  （skip_probability→跳过概率、text→文本内容、weight→权重）
- 实测 mutex_groups 8 组（组0 含 skip_probability + 10 items）、optional_items 4 项正常渲染与收集

### 类别白名单空列表提示

- 9 个类别池（人数与性别/外貌/…）默认空列表只有"+ 添加"按钮，用户不知填什么
- **修复**：空标量列表显示提示"（空列表：点「+ 添加」填入条目，或留空使用默认随机抽样）"

### 创意锚点文本框过窄修复

- **根因**：ListEditor 对象行字段未加入布局 → QLineEdit 塌缩到 35px
- **修复**：字段加入布局后，文本框 **35px → 655-727px**，name/cn/tags 全部正常宽度

### 验证

- 折叠控件扫描 0 处；额外要求池字段全汉化
- 类别白名单 9 个提示；锚点 QLineEdit 655-727px
- 收集完整（锚点 12 条 / mutex_groups 8 组 / 预设保存）；Qt self-test 退出码 0；51 单元测试全过

## v3.3.3 (当前发布候选)

基于 v3.3.2 的高级页**彻底汉化 + 去除折叠 + 修复配置项缺失感知**。

### 完全汉化

- 补全汉化映射：顶层叶子键（内容分级上限 / 最少 tag 数 / 默认输出目录 / 额外要求 /
  r18 最小 tag 数 / r18 指令 等）+ 创意锚点 7 类（超现实场景 / 尺度反转 / 魔法仪式 /
  动漫表现 / 叙事反差 / 不可能动作 / 奇幻元素）
- 审计确认：**全部 357+ 叶子字段无未汉化项**（中文键名除外）；分组标题显示
  `中文名（英文键）` 格式（如"抽样数量（sample_counts）"）

### 去除折叠（章节化后不再需要）

- 章节内顶层键**直接平铺**显示，不再有可折叠小节（删除 CollapsibleSection 包裹、
  移除"全部展开/全部折叠"按钮）
- 嵌套 dict 才显示分组标题（避免叶子键标题重复），叶子键直接显示字段
- 创意锚点章节保留整体懒加载：显示"展开锚点配置"按钮，点击后 78 个锚点按
  7 类平铺（每类标题"超现实场景（12 个）"）

### 修复"配置项缺失"感知

- 根因：默认折叠导致用户展开章节看不到内容（如抽样数量章节折叠后无字段）
- 修复后：抽样数量章节**展开即见 9 个字段**（人数与性别/外貌/服装/姿势动作/
  表情反应/镜头/场景环境/画面质感/创意锚点），全部配置项直接可见

### 验证

- 抽样数量章节 9 字段直接可见；全部字段汉化审计通过
- 预设保存（含 sample_counts.appearance 嵌套）/切换重建/锚点懒加载/深色/缩放全通过
- 枚举汉化还原正常；Qt self-test 退出码 0；51 单元测试全过
