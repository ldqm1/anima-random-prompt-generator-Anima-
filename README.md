...
- **界面功能**：
  ...
  - 「高级」页：**完整配置编辑**——自动加载 `generation_config.yaml` 与
    ...
    悬停任意控件显示该项说明与效果（来自 yaml 注释）；危险区
...
- **高级设置保存**：在「高级」页点「保存设置」会把**修改过的配置项**写入
  `%APPDATA%\AnimaPromptGenerator\user_config.yaml`（用户目录），下次启动/生成时
  合并覆盖默认配置；「恢复默认」删除该文件。exe 打包版的项目 yaml 是只读的，
  所有修改都经用户目录生效。
- **从源码运行 GUI**：
  ```bash
  python anima_gui.py
