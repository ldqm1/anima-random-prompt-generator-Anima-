...
- **界面功能**：
  ...
  - 「API 设置」页：**OpenAI 兼容格式**——支持任意 OpenAI 兼容接口
    （DeepSeek / Moonshot / OpenRouter / 本地 vLLM 等），填写 Base URL + API Key + 模型名即可，
    API Key 可勾选"记住"（保存到用户目录而非项目）、模型可下拉选择常用模型、
    Temperature、超时、思考模式（reasoning_effort），带"测试连接"按钮；
  - 「高级」页：**完整配置编辑**——自动加载 `generation_config.yaml` 与
    `creative_anchors.yaml` 的全部配置项，按数据类型生成控件（数字→步进框、
    开关→勾选、枚举→下拉、列表→可增删编辑器、深层嵌套→递归展开），
    **悬停任意控件（鼠标停留约 0.4 秒）显示该项说明与效果**（357 项 100% 覆盖：
    优先 yaml 注释，无注释时按字段语义自动生成说明）；危险区
    （r18 主题控制 / 额外要求池 / 角色池 / 白名单池 / 子类配额等）默认折叠且**懒加载**
    （折叠时不构建控件，展开才加载，启动快、滚动流畅）；支持滚轮直接滚动；
    支持「保存设置」（只写与默认不同的部分到用户目录，重启生效）、
    「恢复默认」、「全部展开/折叠」；
  - 「配置」页：**多预设管理**——保存多套完整配置（生成参数 + 创意锚点），
...
- **配置保存与预设**：所有自定义配置（生成配置 + 创意锚点）以**预设**为单位存入
  `%APPDATA%\AnimaPromptGenerator\profiles.json`（用户目录）；「高级」页保存写入当前
