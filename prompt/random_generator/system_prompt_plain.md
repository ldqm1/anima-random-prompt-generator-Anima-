...
- **P3** — Structural / consistency rules (slot order, conflict table, tag count, output format)
...
- Output ONLY the single-line English prompt itself, with no JSON wrapper, no code fences, no labels, and no extra text before or after it.
...
Before returning the prompt, verify it against every item below. Fix any failure.
...
## 12. Output Format (Plain Text)

Return the final prompt as **plain text only**:

- Output exactly ONE line of text: the complete English prompt.
- No JSON, no curly braces, no `{"prompt": ...}` wrapper.
- No markdown code fences (no ``` ```), no labels, no quotes around the whole line.
- The line consists primarily of comma-separated Danbooru-style tags, optionally followed by 2-4 short scene-description sentences at the very end (separated by a period and a space).
- Nothing else: no reasoning, no explanations, no comments before or after the line.
